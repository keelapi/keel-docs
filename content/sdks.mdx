...
Install a Keel SDK and send a governed request in your language of choice.

TypeScript is the primary walkthrough below. Python and Swift cover the same surface — see [Also available](#also-available) at the bottom.
...
| Swift | SwiftPM package `https://github.com/keelapi/keel-swift.git`, product `KeelSwift` |
...
| Swift | Apple platform apps and services | Typed clients for permits, executions (sync + stream), execute, proxy, jobs, API keys, and request timeline |
...
Python and Swift cover the same full surface as the TypeScript SDK. Use them when they match your deployment environment better:

- Python: typed sub-clients for permits, executions, execute, proxy, jobs, API keys, and request timeline — with sync and async variants for every method
- Swift: typed clients for permits, executions (sync + stream), execute, proxy, jobs, API keys, and request timeline
