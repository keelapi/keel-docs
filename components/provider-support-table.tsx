        ...
        <table className="x:min-w-full x:border-collapse x:text-sm">
          ...
          <tbody>
            {artifact.providers.map(provider => (
              ...
              >
                <th scope="row" className="x:px-3 x:py-3.5 x:font-normal">
                  <ProviderCell provider={provider.provider} />
                </th>
                {[provider.proxySupported, provider.executionsSupported, provider.streamingSupported].map(
                  (value, index) => (
                    <td
                      key={`${provider.provider}-${index}`}
                      className="x:px-3 x:py-3.5 x:text-center x:text-base x:text-slate-700 x:dark:text-slate-200"
                    >
      ...
      {notedProviders.length > 0 ? (
        <section className="x:mt-8 x:space-y-4">
          ...
          <div className="x:overflow-x-auto">
            <table className="x:min-w-full x:border-collapse x:text-sm">
              <caption className="x:sr-only">
                Provider-specific notes and operation support details.
              </caption>
              <thead>
                <tr className="x:border-b x:border-slate-200 x:text-left x:dark:border-slate-800">
                  <th className="x:w-[12rem] x:px-3 x:py-2.5 x:text-xs x:font-semibold x:uppercase x:tracking-[0.08em] x:text-slate-500 x:dark:text-slate-400">
                    Provider
                  </th>
                  <th className="x:px-3 x:py-2.5 x:text-xs x:font-semibold x:uppercase x:tracking-[0.08em] x:text-slate-500 x:dark:text-slate-400">
                    Notes
              ...
              <tbody>
                {notedProviders.map(provider => (
                  ...
                  >
                    <th scope="row" className="x:px-3 x:py-4 x:font-normal">
                      <ProviderCell provider={provider.provider} emphasize />
                    </th>
                    <td className="x:px-3 x:py-4 x:text-sm x:leading-6 x:text-slate-600 x:dark:text-slate-300">
